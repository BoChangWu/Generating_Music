...
# 載入生成器和鑑別器的權重
generator.load_weights('./model/generator_weights')
discriminator.load_weights('./model/discriminator_weights')
...
with tf.device('/device:GPU:0'):
    hist = gan.fit(X,Y,batch_size=BATCH,epochs=epochs_,verbose=1)
...
midi_encode = Midi_()
midi_encode.make_file(notes=out,name = f'test15_{y}_{epochs_*4}.mid')
